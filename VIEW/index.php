...
<?php if (isset($_SESSION['loginsuccess2'])) { ?>
    ...
    $query = "SELECT COUNT(`id`) as Parcel FROM `post`";
    $result = mysqli_query($conn, $query);
    while ($row = mysqli_fetch_array($result)) {
        $Parcel = $row['Parcel'];
    }
    $query = "SELECT COUNT(id) as USER FROM `user` WHERE role ='ผู้ใช้ทั่วไป'";
    $result = mysqli_query($conn, $query);
    while ($row = mysqli_fetch_array($result)) {
        $USER = $row['USER'];
    }
    $query = "SELECT COUNT(id) as Wait FROM `post`  WHERE status =0";
    $result = mysqli_query($conn, $query);
    while ($row = mysqli_fetch_array($result)) {
        $Wait = $row['Wait'];
    }
    $query = "SELECT COUNT(id) as Success FROM `post`  WHERE status =1";
    $result = mysqli_query($conn, $query);
    while ($row = mysqli_fetch_array($result)) {
        $Success = $row['Success'];
    }
        ...
        <aside class="main-sidebar sidebar-dark-primary elevation-4">
            ...
            <div class='sidebar'>
                ...
                <div class='user-panel mt-3 pb-3 mb-3 d-flex'>
                    ...
                    <div class='info'>
                        <a href='./profile.php' class='d-block'><?php echo $_SESSION['MenuName'] ?> </a>
                    </div>
                </div>
                <!-- Sidebar Menu -->
                <nav class='mt-2'>
           ...
           with font-awesome or any other icon font library -->
                        <li class='nav-item actived'>
                            <a href='./' id='index' class='nav-link active'>
